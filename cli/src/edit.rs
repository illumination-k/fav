...
impl Edit {
    ...
    pub fn edit<S: AsRef<[u8]>>(&self, text: S) -> Result<String> {
        String::from_utf8(self.edit_bytes(text)?).map_err(|_| Error::from(ErrorKind::InvalidData))
    }
    ...
    fn edit_file<P: AsRef<Path>>(&self, file: P) -> Result<()> {
        let (editor, args) = if let Some(editor) = self.editor.as_ref() {
            get_full_editor_cmd(editor.to_string())?
        } else {
            get_editor_args()?
        };
